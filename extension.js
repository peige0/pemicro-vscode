...
'<script>const vscode=acquireVsCodeApi();function send(c){vscode.postMessage({command:c,start:document.getElementById("start").value,end:document.getElementById("end").value});}window.addEventListener("message",e=>document.getElementById("status").textContent=e.data);</script>',
'</body></html>'
].join('');}
